            ...
            <div class="grid grid-cols-2 gap-3 sm:gap-6 md:grid-cols-3 xl:grid-cols-4" data-training-grid>
                @forelse($formationProducts as $product)
                    ...
                    <article class="premium-card group overflow-hidden rounded-[1.5rem] transition hover:-translate-y-2 hover:shadow-premium sm:rounded-[2rem]" data-training-card data-category="{{ $category }}">
                        <div class="relative min-h-[130px] overflow-hidden bg-gradient-to-br from-navy via-royal to-gold sm:min-h-[220px]"><img src="{{ $product->image ? asset('storage/' . $product->image) : asset('/assets/training/crypto-masterclass.jpg') }}" alt="{{ $product->title }}" class="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105"><div class="absolute inset-0 bg-gradient-to-t from-navy/80 via-navy/20 to-transparent"></div><span class="absolute bottom-5 left-5 rounded-full bg-white/10 px-3 py-1 text-xs font-black text-gold backdrop-blur">{{ $category }}</span>@if($product->is_on_promotion)<span class="absolute right-4 top-4 rounded-full bg-rose-600 px-3 py-1 text-xs font-black text-white shadow-premium">Promo</span>@endif</div>
                        ...
                        <div class="flex items-center justify-between gap-3 border-t border-slate-100 p-4 sm:p-6"><x-product-price :product="$product" /><a href="{{ route('products.show', $product) }}" class="rounded-full bg-royal px-4 py-2 text-xs font-black text-white shadow-glow">S'inscrire</a></div>
                    </article>
                @empty
